  ...
  return (
    <>
      ...
      <div className="hidden lg:flex w-56 2xl:w-64 bg-slate-900 text-white h-screen flex-col shadow-lg fixed left-0 top-0 z-50">
        ...
        <div 
          className="p-4 bg-slate-800 border-b border-slate-700 sticky top-28 z-10 cursor-pointer hover:bg-slate-700 transition"
          onClick={() => setActive('profile')}
        >
          <div className="flex items-center gap-3">
            <img
              src={currentUser.avatar || `https://i.pravatar.cc/96?img=${Math.random() * 100}`}
              alt={currentUser.name || 'User'}
              className="w-12 h-12 rounded-full border-2 border-indigo-500 hover:border-indigo-400 transition cursor-pointer"
            />
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-white truncate">
                {currentUser.name || currentUser.fullName}
