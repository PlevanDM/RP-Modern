# Простой скрипт для развертывания RepairHub Pro
...
Write-Host "🔍 Проверяем доступность сервера..." -ForegroundColor Yellow

# Проверяем ping
try {
    $pingResult = Test-Connection -ComputerName $SERVER_IP -Count 1 -Quiet
    if ($pingResult) {
        Write-Host "✅ Сервер доступен (ping работает)" -ForegroundColor Green
    } else {
        Write-Host "❌ Сервер недоступен" -ForegroundColor Red
        exit 1
    }
} catch {
    Write-Host "❌ Ошибка проверки ping: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
