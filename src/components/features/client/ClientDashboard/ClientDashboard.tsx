  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <header className="mb-10">
        <h1 className="text-4xl font-bold text-gray-900 mb-2">Здравствуйте, {currentUser.name}!</h1>
        <p className="text-lg text-gray-600">Ваш личный кабинет</p>
      </header>

      {/* Статистика */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <InfoCard icon={<ClipboardList />} title="Открытые Заказы" value={openOrders} color="blue" />
        <InfoCard icon={<Star />} title="Завершенные Заказы" value={completedOrders} color="green" />
        <Info-card icon={<CreditCard />} title="Всего Потрачено (UAH)" value={`${totalSpent.toLocaleString()}`} color="purple" />
        <InfoCard icon={<MessageSquare />} title="Новые Сообщения" value={3} color="yellow" />
      </div>

      {/* Основной контент */}
      <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">Последние Заказы</h2>
